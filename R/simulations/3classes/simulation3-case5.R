...
simulation3.case4 <- function(){
  ...
  t <- rep(1:40, each = 1)/20  ## time point [1, 1.01, ..., 3.99, 4]
  n1 <- 200                    ## class 1
  n2 <- 200                    ## class 2
  n3 <- 200                    ## class 3
  ...
  phi1 <- function(t){
    #t.s <- t/max(t)
    t.s<-t*10
    phi1.t <- (-2)*cos(pi*(t.s - 1/2))
    return(list(phi1.t = phi1.t))
  }
  
  phi2 <- function(t){
    t.s<-t*10
    #t.s <- t/max(t)
    phi2.t = sin(pi*(t.s - 1/2))
    return(list(phi2.t = phi2.t))
  }
  ...
  for (k in 1:1){
    ...
    for (j in 1:100){
      ...
      for (i in 1:600){     ## i from 1 to 200
        ...
        xi1 <- rnorm(1,0,1)
        xi2 <- rnorm(1,0,1)
        sse <- xi1 * phi1(t)$phi1.t + xi2 * phi2(t)$phi2.t
